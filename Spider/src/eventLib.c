...
//Variables required to support monitoring with the sampling procedure

int monitoring_sampling = 0;
papify_action_s papify_action_sampling[0];
int PE_id_sampling;
struct itimerval timer_sampling;
...
void configure_papify_actor(papify_action_s* papify_action, char* componentName, char* actorName, int num_events, char* all_events_name, char* actor_ids, int eventSets){
	...
	snprintf(all_actor_ids_local, (strlen(actor_ids)+1) * sizeof(char), "%s", actor_ids);
	char* actor_id;
	int i = 0;
	...
	if(strcmp(componentName, "") != 0){
		...
		for(i = 0; i < eventSets; i++){
			...
			if(papify_eventSet_actor[actorIDs[i]] == -1){	
				...
				papify_action[0].papify_eventSets[i] = papify_eventSet_actor[actorIDs[i]];
			}
		}
	}
...
void event_create_eventSet(papify_action_s* papify_action, int element_id, int eventSetPosition, int dynamic) {
	...
	if(dynamic == 0){
		papify_eventSet = papify_action[0].papify_eventSets[eventSetPosition];
	}
	else{
		papify_eventSet = papify_PEs_info[element_id].papify_eventSet_ID_original[papify_action[0].papify_eventSets[eventSetPosition]];
	}
	...
	retval = PAPI_assign_eventset_component( papify_eventSet, PAPI_get_component_index(papify_action[0].component_id[eventSetPosition]));
	...
	for (i = 0; i < papify_action[0].num_counters; i++) {
		...
		if (info.component_index == PAPI_get_component_index(papify_action[0].component_id[eventSetPosition])){
			//printf("Adding %x to %s actor\n", papify_action[0].papify_eventCodeSet[i], papify_action[0].actor_id);
	...
	if(dynamic == 0){
		...
		papify_action[0].papify_eventSets[eventSetPosition] = papify_eventSet;
	}
	else{
		papify_PEs_info[element_id].papify_eventSet_ID_original[papify_action[0].papify_eventSets[eventSetPosition]] = papify_eventSet;
	}
...
void event_init_output_file(papify_action_s* papify_action, char* actorName, char* all_events_name) {
	...
	for(i = 0; i < papify_action[0].num_eventSets; i++){
		snprintf(all_events_name_local, (strlen(all_events_name)+1) * sizeof(char), "%s", all_events_name);
		event_name = strtok(all_events_name_local, ",");

		snprintf(file_name, sizeof file_name, "%s%s%s%s%s", "papify-output/papify_output_", actorName, "_", papify_action[0].component_id[i], ".csv");
		snprintf(output_string, sizeof output_string, "%s", "PE,Actor,tini,tend");
		monitoring = 0;
		papify_action[0].num_counters_eventSet[i] = 0;
					
		for(j = 0; j < papify_action[0].num_counters; j++){
			...
			if (info.component_index == PAPI_get_component_index(papify_action[0].component_id[i])){			
				strcat(output_string, ",");			
				strcat(output_string, event_name);
				monitoring = 1;
				papify_action[0].num_counters_eventSet[i] = papify_action[0].num_counters_eventSet[i] + 1;
			}
			event_name = strtok(NULL, ",");
		}	
		if(monitoring == 1){	
	...
	if(papify_action[0].num_eventSets == 0){		
...
void event_init_papify_actions(papify_action_s* papify_action, char* componentName, char* actorName, int num_events, int eventSets) {
	...
	char* component;
	snprintf(all_components_local, (strlen(componentName)+1) * sizeof(char), "%s", componentName);
	component = strtok(all_components_local, ",");

	papify_action[0].component_id = (char**)malloc(eventSets * sizeof(char*));
	papify_action->papify_eventSets = malloc(sizeof(int) * eventSets);
	for(i = 0; i < eventSets; i++){
		papify_action[0].component_id[i] = (char*)malloc((strlen(component)+1) * sizeof(char));
		snprintf(papify_action[0].component_id[i], (strlen(component)+1) * sizeof(char), "%s", component);
		component = strtok(NULL, ",");
		papify_action[0].papify_eventSets[i] = PAPI_NULL;
	}

	papify_action[0].PE_id = malloc(150 * sizeof(char));

	papify_action[0].num_counters = num_events;
	papify_action[0].num_eventSets = eventSets;
...
void sampling_monitoring(int signo){

	papify_action_s *papify_action = papify_action_sampling; 
	int PE_id = PE_id_sampling;
	int i;
	int retval;

	papify_action[0].time_end_action = PAPI_get_real_usec() - time_zero;
	retval = PAPI_read( papify_PEs_info[PE_id].papify_eventSet_ID, papify_action[0].counterValuesStop );
	if ( retval != PAPI_OK ){
		test_fail( __FILE__, __LINE__, "PAPI_read",retval );
	}
	for(i = 0; i < papify_action[0].num_counters; i++){
	...
	event_write_file(papify_action, PE_id);
	papify_action[0].time_init_action = papify_action[0].time_end_action;
	for(i = 0; i < papify_action[0].num_counters; i++){
		papify_action[0].counterValuesStart[i] = papify_action[0].counterValuesStop[i];
	}
}	

/* 
* Launch a monitoring that will be sampling the monitoring until a stop is called
*/
void event_start_sampling(papify_action_s* papify_action, int PE_id, int period_ms){

	// Check corresponding PE type and eventSet
	int i;
	struct sigaction act;
	memset (&act, 0, sizeof (act));
	act.sa_handler = &sampling_monitoring;
	sigaction (SIGVTALRM, &act, NULL);
	int eventSet_position = -1;
	for(i = 0; i < papify_action[0].num_eventSets; i++){
		if(strcmp(papify_PEs_info[PE_id].PE_type, papify_action[0].component_id[i]) == 0){
			eventSet_position = i;
		}
	}
	...
	if(eventSet_position != -1){
		...
		papify_PEs_info[PE_id].papify_eventSet_ID = papify_PEs_info[PE_id].papify_eventSet_ID_original[papify_action[0].papify_eventSets[eventSet_position]];
		event_launch(papify_action, PE_id);
		monitoring_sampling = 1;
		papify_action[0].time_init_action = PAPI_get_real_usec() - time_zero;
		papify_action_sampling[0] = papify_action[0];
		PE_id_sampling = PE_id;

		timer_sampling.it_value.tv_sec = period_ms/1000000;
		timer_sampling.it_value.tv_usec = period_ms%1000000;
		timer_sampling.it_interval.tv_sec = period_ms/1000000;
		timer_sampling.it_interval.tv_usec = period_ms%1000000;
		/* Start a virtual timer. It counts down whenever this process is
		executing. */
		setitimer (ITIMER_VIRTUAL, &timer_sampling, NULL);
	}
}

/* 
* Stop the sampling monitoring
*/
void event_stop_sampling(papify_action_s* papify_action, int PE_id){

	monitoring_sampling = 0;
	timer_sampling.it_value.tv_sec = 0;
	timer_sampling.it_value.tv_usec = 0;
	timer_sampling.it_interval.tv_sec = 0;
	timer_sampling.it_interval.tv_usec = 0;
	setitimer (ITIMER_VIRTUAL, &timer_sampling, NULL);
}
...
void event_start(papify_action_s* papify_action, int PE_id){
	...
	if(eventSet_position != -1){
		if(papify_PEs_info[PE_id].papify_eventSet_ID_original[papify_action[0].papify_eventSets[eventSet_position]] == papify_PEs_info[PE_id].papify_eventSet_ID && papify_PEs_info[PE_id].papify_eventSet_ID != -1){
			snprintf(papify_action[0].PE_id, (strlen(papify_PEs_info[PE_id].PE_id)+1) * sizeof(char), "%s", papify_PEs_info[PE_id].PE_id);
		...
		else if(papify_PEs_info[PE_id].papify_eventSet_ID_original[papify_action[0].papify_eventSets[eventSet_position]] == -1){
...
void event_write_file(papify_action_s* papify_action, int PE_id){
	...
	if(eventSet_position != -1){
		snprintf(output_file_name, sizeof output_file_name, "%s%s%s%s%s", "papify-output/papify_output_", papify_action[0].actor_id, "_", papify_PEs_info[PE_id].PE_type, ".csv");
		papify_action[0].papify_output_file = fopen(output_file_name,"a+");
		fprintf(papify_action[0].papify_output_file, "%s,%s,%lld,%lld", papify_action[0].PE_id, papify_action[0].actor_id, papify_action[0].time_init_action, papify_action[0].time_end_action);

		for(i = 0; i < papify_action[0].num_counters_eventSet[eventSet_position]; i++){
	...
	} else if (papify_action[0].num_eventSets == 0) {
